...
results_long <- results_norm %>%
  select(Model, Method, AIC_norm, BIC_norm, RMSE_norm, MAE_norm) %>%
  pivot_longer(cols = c(AIC_norm, BIC_norm, RMSE_norm, MAE_norm),
...
comparison_long <- comparison %>%
  select(Model, AIC_Diff, BIC_Diff, RMSE_Diff) %>%
  pivot_longer(cols = c(AIC_Diff, BIC_Diff, RMSE_Diff),
...
cat("================================================================\n\n")
cat("Files created:\n")
list.files(plots_dir, pattern = "\\.png$", full.names = FALSE) %>%
  paste0("  - ", .) %>%
  cat(sep = "\n")
cat("\n")

...
summary_stats <- data.frame(
  ...
  Value = c(
    ...
    moments::kurtosis(prices) - 3  # Excess kurtosis
  )
)
...
fit_and_compare <- function(prices, order, model_name) {
  ...
  if (!is.null(fit_css)) {
    ...
    results <- rbind(results, data.frame(
      ...
      Skewness = moments::skewness(res_css),
      Kurtosis = moments::kurtosis(res_css) - 3
    ))
  }
  ...
  if (!is.null(fit_pmm2)) {
    ...
    results <- rbind(results, data.frame(
      ...
      Skewness = moments::skewness(res_pmm2),
      Kurtosis = moments::kurtosis(res_pmm2) - 3
    ))
  }
